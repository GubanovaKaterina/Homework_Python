...
def create_group_student_table():
    sql = text("""CREATE TABLE group_student
...
def update_group_student(user_id, new_group_id):
    sql = text(
                "UPDATE group_student SET group_id = "
                ":new_group_id WHERE user_id = :user_id"
                )
    with db.connect() as connection:
        connection.execute(sql, {"new_group_id": new_group_id,
                                 "user_id": user_id})
